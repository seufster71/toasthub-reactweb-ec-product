...
export default function ECProductView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption, 
	closeModal, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.EC_PRODUCT_PAGE != null) {
    	columns = itemState.prefLabels.EC_PRODUCT_PAGE;
    }
    let group = "TABLE1";
    
    let header = "";
	if (itemState.prefTexts.EC_PRODUCT_PAGE != null && itemState.prefTexts.EC_PRODUCT_PAGE.EC_PRODUCT_PAGE_HEADER != null) {
		header = itemState.prefTexts.EC_PRODUCT_PAGE.EC_PRODUCT_PAGE_HEADER.value;
	}
